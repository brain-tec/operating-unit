...
class StockMove(models.Model):
    ...
    def _generate_valuation_lines_data(self, partner_id, qty, debit_value, credit_value, debit_account_id,
        ...
        if res:
            ...
            if (self.operating_unit_id and self.operating_unit_dest_id and self.operating_unit_id !=
                    self.operating_unit_dest_id and debit_line_vals['account_id'] != credit_line_vals['account_id']
            ):
            ...
            debit_line_vals['operating_unit_id'] = ou_id or self.operating_unit_dest_id.id or self.\
                operating_unit_id.id
            credit_line_vals['operating_unit_id'] = ou_id or self.operating_unit_id.id or self.\
                operating_unit_dest_id.id
            rslt = {'credit_line_vals': credit_line_vals,
                    'debit_line_vals': debit_line_vals}
            if price_diff_line_vals:
                price_diff_line_vals['operating_unit_id'] = ou_id or self.operating_unit_id.id or self.\
                    operating_unit_dest_id.id
    ...
    def _prepare_account_move_line(self, qty, cost, credit_account_id, debit_account_id, description):
        ...
        if res:
            ...
            if (self.operating_unit_id and self.operating_unit_dest_id and
            ...
            debit_line_vals['operating_unit_id'] = (self.operating_unit_dest_id.id or self.operating_unit_id.id)
            credit_line_vals['operating_unit_id'] = (self.operating_unit_id.id or self.operating_unit_dest_id.id)
        ...
        for move in self:
            ...
            if move.product_id.valuation == 'real_time':
                ...
                if (
                    ...
                    src_company_ctx = dict(force_company=move.location_id.company_id.id)
                    company_ctx = dict(company_id=move.company_id.id)
                    self = self.with_context(src_company_ctx)
                    (journal_id, acc_src, acc_dest, acc_valuation) = move._get_accounting_data_for_valuation()

                    move_lines = move._prepare_account_move_line(move.product_qty, move.product_id.standard_price,
                                                                 acc_valuation, acc_valuation, move.name)
